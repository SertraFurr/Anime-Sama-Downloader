...
def main():
    ...
    if not check_package(ask_install=True, first_run=True):
        ...
        if ask_user in ['y', 'yes', '1']:
            if not check_package(ask_install=True, first_run=False):
                print_status("Failed to install required packages. Please install them manually and re-run the script.",
                             "error")
                sys.exit(1)
        else:
    ...
    if not check_ffmpeg_installed():
        print_status(
            "FFmpeg is not installed or not found in the PATH. You could consider installing it from https://ffmpeg.org/download.html",
            "error")
    ...
    try:
        ...
        if not base_url:
            ...
            while True:
                ...
                if mode == '1':
                    ...
                    while True:
                        choice = input(
                            f"{Colors.BOLD}Select anime (1-{len(results)}) or 'c' to cancel: {Colors.ENDC}").strip()
                                ...
                                if options:
                                    ...
                                    if anime_opts:
                                        ...
                                        for opt in anime_opts:
                                            print(
                                                f"{Colors.OKCYAN}{idx_counter}. {opt['name']} ({opt['url']}){Colors.ENDC}")
                                            idx_counter += 1
                                    ...
                                    if scan_opts:
                                        ...
                                        for opt in scan_opts:
                                            print(
                                                f"{Colors.OKBLUE}{idx_counter}. {opt['name']} ({opt['url']}){Colors.ENDC}")
                                            idx_counter += 1

                                    while True:
                                        s_choice = input(
                                            f"{Colors.BOLD}Select season (1-{len(options)}): {Colors.ENDC}").strip()
                                        if s_choice.isdigit():
                                    ...
                                    print_status("This page doesn't seem to contain any anime downloadable content.",
                                                 "warning")
                                    continue
                    if valid_choice:
                        break
        ...
        if not is_valid:
            ...
            if season_options:
                ...
                print_status("Could not find any seasons/versions. Please define one manually (url/saison...)",
                             "warning")
                ...
                if not player_choice:
                    ...
                    if target in domain_map:
                        ...
                        if isinstance(val, list):
                            search_domains.extend(val)
                        else:
                            search_domains.append(val)
                    else:
        ...
        if interactive:
            if len(episode_indices) > 1 and not args.threads:
                thread_choice = input(
                    f"{Colors.BOLD}Download all episodes simultaneously? (t/1/y = yes / s = no): {Colors.ENDC}").strip().lower()
            ...
            if any('m3u8' in src for src in video_sources if src):
                ...
                if not args.fast:
                    ts_thread_choice = input(
                        f"{Colors.BOLD}Download .ts files simultaneously (fast)? (y/n): {Colors.ENDC}").strip().lower()
                    use_ts_threading = ts_thread_choice in ['t', 'threaded', '1', 'y', 'yes']

                if not args.mp4:
                    auto_mp4_choice = input(
                        f"{Colors.BOLD}Convert to .mp4 automatically? (y/n): {Colors.ENDC}").strip().lower()
        ...
        try:
            if use_threading and len(episode_indices) > 1:
                ...
                with ThreadPoolExecutor() as executor:
                    future_to_episode = {
                        executor.submit(download_episode, ep_num, url, video_src, get_anime_name, save_dir,
                                        use_ts_threading, automatic_mp4, pre_selected_tool, args.no_mal,
                                        interactive): ep_num
                ...
                for episode_num, url, video_source in zip(episode_numbers, urls, video_sources):
                    success, _ = download_episode(episode_num, url, video_source, get_anime_name, save_dir,
                                                  use_ts_threading, automatic_mp4, pre_selected_tool, args.no_mal,
                                                  interactive)
                    if not success: failed_downloads += 1
